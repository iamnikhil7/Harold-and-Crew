...
export default function OnboardingPage() {
  ...
  return (
    <div className={`min-h-[100dvh] mx-auto relative overflow-hidden ${isDesktop ? "min-h-screen" : "max-w-[430px]"}`} style={{ background: "linear-gradient(180deg, #F8F3EC 0%, #F2EBE0 60%, #E8D5B8 100%)" }}>
      <AnimatePresence mode="wait">
        {/* ── WELCOME ──────────────────────────────────────────── */}
        {phase === "welcome" && (
          <motion.div key="welcome" variants={stepTransition} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.4 }} className={`h-[100dvh] flex flex-col justify-between relative overflow-hidden ${isDesktop ? "items-center justify-center px-8" : "px-5 pb-8"}`} style={{ paddingTop: isDesktop ? "0" : "env(safe-area-inset-top, 48px)" }}>
            ...
            <div className={`relative z-10 ${isDesktop ? "max-w-2xl mx-auto text-center flex flex-col items-center" : ""}`}>
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                <Image src="/harold-mascot.png" alt="Harold" width={isDesktop ? 100 : 60} height={isDesktop ? 100 : 60} className="rounded-2xl shadow-lg mb-6" />
              </motion.div>
              <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className={`leading-tight ${isDesktop ? "text-6xl mb-4" : "text-4xl"}`} style={{ fontFamily: "'DM Serif Display', Georgia, serif", color: "#2A2014" }}>
                Hey there,<br />I&apos;m Harold
              </motion.h1>

              <motion.h2 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className={`leading-tight ${isDesktop ? "text-4xl mt-4 mb-8" : "text-3xl"}`} style={{ fontFamily: "'DM Serif Display', Georgia, serif", fontStyle: "italic", color: "#2A2014" }}>
                Let&apos;s get to know{isDesktop ? " " : <br />}each other!
              </motion.h2>
              <motion.button initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setPhase("assessment")} className={`py-4 rounded-2xl text-base font-semibold flex items-center justify-center gap-2 ${isDesktop ? "w-80 mt-4" : "w-full"}`} style={{ background: "#2A2014", color: "#F8F3EC" }}>
        ...
        {phase === "assessment" && (
          <motion.div key={`q-${currentQ}`} variants={stepTransition} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.35 }} className={`h-[100dvh] flex flex-col pb-6 ${isDesktop ? "max-w-3xl mx-auto px-8 pt-12 justify-center" : "px-5"}`} style={{ paddingTop: isDesktop ? undefined : "env(safe-area-inset-top, 48px)" }}>
            {/* Header */}
            ...
            {/* Question prefix */}
            {q.prefix && <p className={`italic mb-1 ${isDesktop ? "text-base" : "text-sm"}`} style={{ color: "#8B7355", fontFamily: "'DM Serif Display', Georgia, serif" }}>{q.prefix}</p>}

            {/* Question text */}
            <h1 className={`leading-tight mb-6 ${isDesktop ? "text-4xl" : "text-2xl sm:text-3xl"}`} style={{ fontFamily: "'DM Serif Display', Georgia, serif", fontStyle: "italic", color: "#2A2014" }}>
              {q.text}
            </h1>
            ...
            <div className="flex-1 overflow-y-auto">
              {/* Image cards */}
              {q.type === "image" && (
                <div className={`grid gap-3 mb-4 ${isDesktop ? (q.options.length > 4 ? "grid-cols-3" : "grid-cols-2") : "grid-cols-2"}`}>
                  {q.options.map((opt) => (
                    <motion.button key={opt.value} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => handleSelect(q.id, opt.value)}
                      ...
                      <Image src={opt.img!} alt={opt.label} fill className="object-cover" sizes={isDesktop ? "33vw" : "50vw"} />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/10 to-transparent" />
                      <span className={`absolute bottom-3 left-3 right-3 font-semibold text-white leading-tight ${isDesktop ? "text-base" : "text-sm"}`} style={{ fontFamily: "'DM Serif Display', Georgia, serif" }}>{opt.label}</span>
              ...
              {q.type === "illustration" && (
                <div className={`grid gap-3 mb-4 ${isDesktop ? "grid-cols-4" : "grid-cols-2"}`}>
                  {q.options.map((opt) => (
                    <motion.button key={opt.value} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => handleSelect(q.id, opt.value)}
              ...
              {q.type === "text" && (
                <div className={`grid gap-3 mb-4 ${isDesktop ? "grid-cols-3" : "grid-cols-2"}`}>
                  {q.options.map((opt) => (
                    <motion.button key={opt.value} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => handleSelect(q.id, opt.value)}
              ...
              <div className="mt-4 mb-4">
                <input type="text" value={somethingElse} onChange={(e) => setSomethingElse(e.target.value)} placeholder="Something else?" className={`w-full px-4 py-3.5 rounded-2xl border text-sm bg-white/80 placeholder:text-[#B8A88A] focus:outline-none focus:border-[#8B7355] transition-colors ${isDesktop ? "max-w-md" : ""}`} style={{ borderColor: "#D4C4A8", color: "#2A2014" }} />
              </div>
            </div>

            {/* Bottom: Harold peek (only on last question) + Next button */}
            <div className={`relative mt-2 ${isDesktop ? "max-w-md mx-auto w-full" : ""}`}>
        ...
        {/* ── HEALTH CONNECTION ──────────────────────────────── */}
        {phase === "health" && (
          <motion.div key="health" variants={stepTransition} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.35 }} className={`h-[100dvh] flex flex-col pb-6 relative ${isDesktop ? "max-w-2xl mx-auto px-8 pt-12 justify-center" : "px-5"}`} style={{ paddingTop: isDesktop ? undefined : "env(safe-area-inset-top, 48px)" }}>
            ...
            <motion.div animate={{ y: [0, -6, 0], rotate: [0, 5, 0] }} transition={{ duration: 3, repeat: Infinity }} className="absolute top-12 right-4 z-10">
              <Image src="/harold-mascot.png" alt="Harold" width={isDesktop ? 90 : 70} height={isDesktop ? 90 : 70} className="rounded-2xl shadow-lg opacity-80" />
            </motion.div>

            <div className="flex-1 flex flex-col">
              <h1 className={`leading-tight mt-4 mb-3 ${isDesktop ? "text-3xl max-w-lg" : "text-2xl max-w-[280px]"}`} style={{ fontFamily: "'DM Serif Display', Georgia, serif", fontStyle: "italic", color: "#2A2014" }}>
                Want Harold to observe your patterns?
              </h1>
              <p className={`mb-6 leading-relaxed ${isDesktop ? "text-sm max-w-lg" : "text-xs max-w-[300px]"}`} style={{ color: "#8B7355" }}>
                He can look at your health, sleep, recovery, &amp; activity data to help you understand what it means in your daily life.
              </p>

              {/* Connection list */}
              <div className={`bg-white/70 rounded-2xl overflow-hidden shadow-sm mb-4 ${isDesktop ? "max-w-lg" : ""}`}>
                {[
        ...
        {/* ── IDENTITY ANCHOR ──────────────────────────────── */}
        {phase === "anchor" && (
          <motion.div key="anchor" variants={stepTransition} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.35 }} className={`h-[100dvh] flex flex-col items-center justify-center pb-8 ${isDesktop ? "px-8" : "px-5"}`} style={{ paddingTop: isDesktop ? "0" : "env(safe-area-inset-top, 48px)" }}>
            <div className={`w-full ${isDesktop ? "max-w-2xl" : "max-w-xl"}`}>
        ...
        {/* ── ARCHETYPE REVEAL ───────────────────────────────── */}
        {phase === "reveal" && (
          <motion.div key="reveal" variants={stepTransition} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.35 }} className={`h-[100dvh] flex flex-col items-center justify-center pb-8 ${isDesktop ? "px-8" : "px-5"}`} style={{ paddingTop: isDesktop ? "0" : "env(safe-area-inset-top, 48px)" }}>
            <div className={`w-full text-center ${isDesktop ? "max-w-2xl" : "max-w-xl"}`}>
